...
function ReadMore(props: propsString | propsArray): ReactElement {
  ...
  const Read = <div style={{cursor: "pointer", textDecoration: "underline"}} onClick={() => setDesc(!desc)}>{desc ? `Свернуть` : `Показать полностью`}</div>
  ...
  if (Array.isArray(longDesc) && Array.isArray(component)) {
    ...
    return <RenderWrap>
      {desc 
        ? component.map(e => renderItem(e))
        : longDesc.map(e => renderItem(e))}
      {showButton && Read}
    </RenderWrap>
  }
