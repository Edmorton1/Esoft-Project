...
import { PostsDTO, PostsDTOSchema } from "@t/gen/dtoObjects";
import { Posts } from "@t/gen/Users";
import { ExpressMulterFileSchema, zid } from "@t/shared/zodSnippets";
import { z } from "zod";

interface IPostsValidation {
	get: (ctx: HttpContext) => [number, number?];
	post: (ctx: HttpContext) => PostsDTO;
	put: (ctx: HttpContext) => [number, PostsDTO];
...
class PostsValidation implements IPostsValidation {
    ...
    // logger.info({PUT_DTO: { ...dto, files, userid }})
		...
		const parsed = PostsDTOSchema.parse({ ...dto, files, userid });

    const id = zid.parse(ctx.params.post_id)
